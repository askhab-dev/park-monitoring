...
export const PopulatItems = () => {
  const { data, error, isLoading } = useApi<SalesByProductResponse>(
  ...
  const totalPercentage =
    100 - topProducts.reduce((sum, item) => sum + item.percentageOfAllSales, 0);
        ...
        <div className={styles.card}>
          <div className={styles.cardTop}>
            ...
            <div className={styles.percentBadge}>
              {100 - Math.round((soldInTopFive / totalSold) * 100)}%
            </div>
          </div>
          <div className={styles.cardSubtitle}>
            <br />
