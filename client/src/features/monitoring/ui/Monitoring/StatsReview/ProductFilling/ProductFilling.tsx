...
export const ProductFilling = () => {
  const { data, error, isLoading } = useApi<ProductFillResponse>(
    '/machines/product-fill',
  );
  ...
  const sortedAsc = [...topFilled].sort(
    (a, b) => a.fillPercentage - b.fillPercentage,
  );
          ...
          return (
            <div key={idx} className={styles.barColumn}>
              <div className={styles.barBackground}>
                ...
                >
                  {isLow && (
                    <div className={styles.barCount}>{it.itemCount}</div>
                  )}
                </div>
              </div>
            </div>
